...
import org.arquillian.example.infrastructure.MySqlDataSource;
...
public class BeerRepositoryTest {
    ...
    @Deployment(name= "datasource", testable = false, order = 1)
    public static Archive<?> createDataSource() {
        final File[] jars = Maven.configureResolver().workOffline().loadPomFromFile("pom.xml").resolve("mysql:mysql-connector-java:5.1.35").withoutTransitivity().asFile();
        final JavaArchive mysqlDriver = ShrinkWrap.createFromZipFile(JavaArchive.class, jars[0]);

        return ShrinkWrap.create(WebArchive.class, "datasource-registration.war")
                .addClass(MySqlDataSource.class)
                .addAsLibraries(mysqlDriver);
    }
    ...
    //# end::arquillian_cube_jpa_test_2[]
